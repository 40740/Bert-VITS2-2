...
def apply_lora(layer: nn.Module, register=True, merge=False, lora_config=default_lora_config):
    """add lora parametrization to a layer, designed to be used with model.apply"""
    # TODO: 目前必须原模型不可以有重参数化部分，不然直接寄。之后可以考虑修复一下
    # TODO: 这里不用unsafe会出问题，暂时冒个险
    if len(layer._forward_pre_hooks) > 0:
        return  # TODO: 这里暂时把已经有hook的层跳过，因为不知道怎么对其加重参数化
    if register:
        if type(layer) in lora_config:
            for attr_name, parametrization in lora_config[type(layer)].items():
                try:
                    ...
                    print(f"Failed to add LoRA to {layer.__class__.__name__}.{attr_name}: {e}")
